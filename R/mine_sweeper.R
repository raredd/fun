...
#' 
#' @author Yixuan Qiu \email{yixuan.qiu@@cos.name}, Robert Redd (modifications)
...
mine_sweeper <- function(level = 'beginner', width, height, 
                         mines, cheat = FALSE, seed) {
  
  if (all(missing(width), missing(height), missing(mines)))
    switch(level,
           beginner = evalq({width = 9; height = 9; mines = 10}),
           intermediate = evalq({width = 16; height = 16; mines = 40}),
           advanced = evalq({width = 16; height = 20; mines = 60}),
           expert = evalq({width = 16; height = 30; mines = 99}),
           insane = evalq({width = 16; height = 30; mines = 250}),
           stop(paste0('\n\nChoose a skill level:\n\n',
                       'beginner, intermediate, advanced, expert, insane')))
  
  
  # Deal with some exceptions
  if (!interactive()) return()
  
  #   if ('Darwin' %in% Sys.info())
  #     quartz() else x11()
  x11()
  ...
  # Plot a grid
  plot.grid <- function(x, y, w = 1, h = 1, 
                        col1 = 'grey75', col2 = 'grey80', slices = 10) {
    ...
    #                 border = '#777777', lwd = 1)
    # Border
    polygon(x + c(-0.5, -0.5, 0.5, 0.5) * w,
            y + c(-0.5, 0.5, 0.5, -0.5) * h,
            border = '#777777')
  }
  ...
  plot.num <- function(x, y, num) {
    for(i in 1:length(x))  plot.grid(x[i], y[i], 
                                     col1 = '#FFFFFF', col2 = '#C8C8C8')
    pnum = num[num > 0]
    px = x[num > 0]
    py = y[num > 0]
  ...
  plot.mine.explode <- function(x, y) {
    ...
    symbols(x, y, stars = star, inches = FALSE, bg = 'red', fg = NA,
            add = TRUE)
    symbols(x, y, stars = 0.7 * star, inches = FALSE, bg = 'yellow',
            fg = NA, add = TRUE)
  }
  ...
  search.zeroes <- function(pos, mat) {
    ...
    areas <- c(pos, (x > 1 & y > 1) * (pos - nr - 1), 
               (y > 1) * (pos - nr), (x < nr & y > 1) * (pos - nr + 1),
               (x > 1) * (pos - 1), (x < nr) * (pos + 1), 
               (x > 1 & y < nc) * (pos + nr - 1), (y < nc) * (pos + nr),
               (x < nr & y < nc) * (pos + nr + 1))
        ...
        if (current.mat == -1) {
          ...
          while (TRUE) {
            ...
            if (length(pos) == length(temp)) {
              ...
              plot.num(areas.col, height + 1 - areas.row,
                       mine.mat[areas])
              ms[areas] <- 1
              break
            }
