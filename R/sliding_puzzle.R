...
sliding_puzzle <- function(size = c(3, 3), bg = "lightblue", z) {
  ...
  if (!missing(z)) {
      ...
      if (!missing(size))
          warning('Because \"z\" is specified, parameter \"size\"',
                  ' will be ignored.')
  }
  ...
  if (is.null(z)) {
      ...
      if ((len.right %% 2) != (len.z %% 2))
          stop('The sliding puzzle is not solvable!')
  }
  ...
  replot <- function(z) {
      ...
      plot(c(0, m), c(0, n), type = 'n', axes = FALSE, asp = 1, ann = FALSE)
      ...
      text(0.5 + rep(0:(m - 1), each = n), 
           0.5 + rep((n - 1):0, m), as.vector(z), cex = 3)
  }
